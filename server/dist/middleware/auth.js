...
const requireSignIn = async (req, res, next) => {
  ...
  try {
    ...
    res.status(401);
    return next(new utils_class_1.default(err, 401));
  }
};
...
const isAdmin = async (req, res, next) => {
  try {
    ...
    if (user.role !== "admin") {
      return next(
        new utils_class_1.default(
          "Authentication failed, Need admin access",
          401,
        ),
      );
    }
    ...
    return next(new utils_class_1.default("Something went wrong" + err, 500));
  }
};
